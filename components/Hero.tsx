...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col overflow-hidden bg-ink grid-bg">
      {/* Ambient glows — subtle, adapt with theme */}
      <div
      ...
      <div className="relative z-10 flex flex-col flex-1 w-full max-w-7xl mx-auto px-6 sm:px-10 lg:px-16 pt-28 pb-12">
        ...
        {/* Thin rule */}
        <hr className="h-rule mb-10 sm:mb-14" style={{ animation: 'fadeIn 0.5s ease-out 0.1s both' }} />

        {/* MAIN */}
        <div className="flex-1 flex flex-col justify-center">
          {/* Cycling role */}
          <div className="mb-5 h-5 overflow-hidden" style={{ animation: 'fadeUp 0.6s ease-out 0.15s both' }}>
            <span
          ...
          <div style={{ animation: 'fadeUp 0.7s ease-out 0.25s both' }}>
            <h1 className="font-display font-extrabold leading-[0.92] tracking-tight">
              <span className="block text-c-text" style={{ fontSize: 'clamp(2.8rem, 8.5vw, 7.5rem)' }}>
                BUILDING SYSTEMS
              </span>
              <span className="block text-c-cyan" style={{ fontSize: 'clamp(2.8rem, 8.5vw, 7.5rem)' }}>
                THAT HOLD.
              </span>
            </h1>
          </div>
        ...
        {/* Thin rule */}
        <hr className="h-rule mt-10 sm:mt-14 mb-7" style={{ animation: 'fadeIn 0.6s ease-out 0.6s both' }} />
